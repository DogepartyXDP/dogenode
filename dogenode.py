...
PROJECT_NAME = "dogenode"
CURDIR = os.getcwd()
SCRIPTDIR = os.path.dirname(os.path.realpath(__file__))
DOGENODE_CONFIG_FILE = ".dogenode.config"
DOGENODE_CONFIG_PATH = os.path.join(SCRIPTDIR, DOGENODE_CONFIG_FILE)
...
HOST_PORTS_USED = {
    'base': [8335, 18335, 8435, 18435, 4005, 14005, 8095, 18095, 8096, 18096],
    'base_extdoge': [8435, 18435, 4005, 14005, 8095, 18095, 8096, 18096],
    'dogeblock': [8335, 18335, 8435, 18435, 4005, 14005, 8095, 18095, 8096, 18096, 4105, 14105, 27017],
    'full': [8335, 18335, 8435, 18435, 4005, 14005, 8095, 18095, 8096, 18096, 4105, 14105, 27017, 80, 443]
}
VOLUMES_USED = {
...
CONFIGCHECK_FILES_BASE_EXTERNAL_DOGECOIN = [
    ...
    ['addrindexrs-dc', 'addrindexrs-dc.testnet.env.default', 'addrindexrs-dc.testnet.env'],
    ['dogeparty', 'client.conf.default', 'client.conf'],
    ['dogeparty', 'client.testnet.conf.default', 'client.testnet.conf'],
    ['dogeparty', 'server.conf.default', 'server.conf'],
    ['dogeparty', 'server.testnet.conf.default', 'server.testnet.conf'],
];
CONFIGCHECK_FILES_BASE = [
    ['dogecoin', 'dogecoin.conf.default', 'dogecoin.conf'],
    ['dogecoin', 'dogecoin.testnet.conf.default', 'dogecoin.testnet.conf'],
    ['addrindexrs-dc', 'addrindexrs-dc.env.default', 'addrindexrs-dc.env'],
    ['addrindexrs-dc', 'addrindexrs-dc.testnet.env.default', 'addrindexrs-dc.testnet.env'],
    ['dogeparty', 'client.conf.default', 'client.conf'],
    ['dogeparty', 'client.testnet.conf.default', 'client.testnet.conf'],
...
CONFIGCHECK_FILES_DOGEBLOCK = CONFIGCHECK_FILES_BASE + [
    ['dogeblock', 'server.conf.default', 'server.conf'],
    ['dogeblock', 'server.testnet.conf.default', 'server.testnet.conf'],
]
CONFIGCHECK_FILES_FULL = CONFIGCHECK_FILES_DOGEBLOCK;
CONFIGCHECK_FILES = {
    'base_extdoge': CONFIGCHECK_FILES_BASE_EXTERNAL_DOGECOIN,
    'base': CONFIGCHECK_FILES_BASE,
    'dogeblock': CONFIGCHECK_FILES_DOGEBLOCK,
...
def parse_args():
    parser = argparse.ArgumentParser(prog='dogenode', description='dogenode utility v{}'.format(VERSION))
    ...
    parser_install = subparsers.add_parser('install', help="install dogenode services")
    parser_install.add_argument("config", choices=['base', 'base_extdoge', 'dogeblock', 'full'], help="The name of the service configuration to utilize")
    ...
    parser_install.add_argument("--ssh-key", action="store", nargs=1, help="The ecdsa ssh key to download projects from github")
    ...
    parser_uninstall = subparsers.add_parser('uninstall', help="uninstall dogenode services")

    parser_start = subparsers.add_parser('start', help="start dogenode services")
    parser_start.add_argument("services", nargs='*', default='', help="The service or services to start (or blank for all services)")

    parser_stop = subparsers.add_parser('stop', help="stop dogenode services")
    parser_stop.add_argument("services", nargs='*', default='', help="The service or services to stop (or blank for all services)")

    parser_restart = subparsers.add_parser('restart', help="restart dogenode services")
    parser_restart.add_argument("services", nargs='*', default='', help="The service or services to restart (or blank for all services)")

    parser_reparse = subparsers.add_parser('reparse', help="reparse a dogeparty-server or dogeblock service")
    parser_reparse.add_argument("service", choices=REPARSE_CHOICES, help="The name of the service for which to kick off a reparse")

    parser_rollback = subparsers.add_parser('rollback', help="rollback a dogeparty-server")
    ...
    parser_validate = subparsers.add_parser('validate', help="makes a database integrity check in dogeparty-server")
    parser_validate.add_argument("service", choices=VALIDATE_CHOICES, help="The name of the service to make the integrity check")

    parser_vacuum = subparsers.add_parser('vacuum', help="vacuum the dogeparty-server database for better runtime performance")
    ...
    parser_tail = subparsers.add_parser('tail', help="tail dogenode logs")
    ...
    parser_logs = subparsers.add_parser('logs', help="tail dogenode logs")
    ...
    parser_update = subparsers.add_parser('update', help="upgrade dogenode services (i.e. update source code and restart the container, but don't update the container itself')")
    ...
    parser_rebuild = subparsers.add_parser('rebuild', help="rebuild dogenode services (i.e. remove and refetch/install docker containers)")
...
def write_config(config):
    cfg_file = open(DOGENODE_CONFIG_PATH, 'w')
...
def is_container_running(service, abort_on_not_exist=True):
    try:
        container_running = subprocess.check_output('{} docker inspect --format="{{{{ .State.Running }}}}" dogenode_{}_1'.format(SUDO_CMD, service), shell=True).decode("utf-8").strip()
...
def main():
    ...
    config_existed = os.path.exists(DOGENODE_CONFIG_PATH)
    config = configparser.ConfigParser()
    if not config_existed:
        if args.command != 'install':
            print("config file {} does not exist. Please run the 'install' command first".format(DOGENODE_CONFIG_FILE))
            sys.exit(1)
    ...
    assert os.path.exists(DOGENODE_CONFIG_PATH)
    config.read(DOGENODE_CONFIG_PATH)
    ...
    os.environ['DOGENODE_RELEASE_TAG'] = 'latest' if repo_branch == 'master' else repo_branch
    ...
    if args.command == 'install':
        ...
        REPOS = REPOS_BASE if build_config == 'base' else (REPOS_DOGEBLOCK if build_config == 'dogeblock' else REPOS_FULL)
        for repo in REPOS:
        ...
        os.remove(DOGENODE_CONFIG_PATH)
        ...
        os.system("{} docker exec -i -t dogenode_{}_1 bash -c {}".format(SUDO_CMD, args.service, cmd))
    elif args.command == 'shell':
        container_running = is_container_running(args.service)
        if container_running:
            os.system("{} docker exec -i -t dogenode_{}_1 bash".format(SUDO_CMD, args.service))
        else:
        ...
        while services_to_update:
            ...
            if service_base not in git_has_updated:
                ...
                if service_base == 'dogeparty':  # special case
                    service_dirs = [os.path.join(SCRIPTDIR, "src", "dogeparty-lib"), os.path.join(SCRIPTDIR, "src", "dogeparty-cli")]
                else:
                ...
                for service_dir in service_dirs:
                    ...
                    if service_base in ('dogeparty', 'dogeblock', 'armory-utxsvr'):
                ...
                if service_base == 'dogewallet' and os.path.exists(os.path.join(SCRIPTDIR, "src", "dogewallet")):  # special case
                    transifex_cfg_path = os.path.join(os.path.expanduser("~"), ".transifex")
                    if os.path.exists(transifex_cfg_path):
                        os.system("{} docker cp {} dogenode_dogewallet_1:/root/.transifex".format(SUDO_CMD, transifex_cfg_path))
                    os.system("{} docker exec -i -t dogenode_dogewallet_1 bash -c \"cd /dogewallet/src ".format(SUDO_CMD) +
                              "&& bower --allow-root update && cd /dogewallet && npm update && grunt build\"")
